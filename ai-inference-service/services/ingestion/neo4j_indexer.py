...
class Neo4jIndexer:
    ...
    def setup_database(self):
        """Yêu cầu 11, 12, 14: Tạo Constraints, Index và Fulltext Index"""
        print("Đang thiết lập Schema (Constraints & Indexes)...")
        queries = [
            # 12. Constraints (Tự động tạo luôn B-tree Index cho name)
            ...
            # 14. Fulltext Indexes để GraphRAG search mờ siêu tốc
    ...
    def clear_database(self):
        with self.driver.session() as session:
            ...
            print("Đã dọn dẹp sạch Neo4j Database.")

    def process_diseases(self):
        folder = os.path.join(DATA_DIR, "diseases")
        if not os.path.exists(folder): return

        print("\n--- Đang nạp Bệnh (Diseases) bằng Batch UNWIND ---")
        batch_data = []
        ...
        for filename in os.listdir(folder):
            ...
            # Yêu cầu 1 & 2: Gom properties và synonyms
            props = {
        ...
        # Yêu cầu 13: Thực thi Batch Insert (Nhanh hơn rất nhiều)
        with self.driver.session() as session:
            # 1. Tạo Node Disease
            session.run("""
                UNWIND $batch AS row
            ...
            # 2. Tạo các Relationship (Yêu cầu 5, 6, 7, 8, 9, 10)
            session.run("""
                UNWIND $batch AS row
                ...
                // TREATED_BY (Yêu cầu 10 - Chiều Disease -> Drug)
                FOREACH (drug IN row.drugs | 
                    MERGE (dr:Drug {name: drug}) 
                    MERGE (d)-[:TREATED_BY]->(dr))
                    
                // AFFECTS BodyLocation
                FOREACH (loc IN row.body_locations | 
                    MERGE (b:BodyLocation {name: loc}) 
                    MERGE (d)-[:AFFECTS]->(b))
                    
                // HAS_RISK_FACTOR
                FOREACH (risk IN row.risk_factors | 
                    MERGE (r:RiskFactor {name: risk}) 
                    MERGE (d)-[:HAS_RISK_FACTOR]->(r))
                    
                // TRIGGERED_BY
                FOREACH (trig IN row.triggers | 
                    MERGE (t:Trigger {name: trig}) 
                    MERGE (d)-[:TRIGGERED_BY]->(t))
                    
                // COMPLICATES_TO
                FOREACH (comp IN row.complications | 
                    MERGE (c:Complication {name: comp}) 
                    MERGE (d)-[:COMPLICATES_TO]->(c))
                    
                // PREVENTED_BY
                FOREACH (prev IN row.prevention | 
                    MERGE (p:Prevention {name: prev}) 
                    MERGE (d)-[:PREVENTED_BY]->(p))
                    
                // DIFFERENTIAL_DIAGNOSIS (Bệnh vs Bệnh)
                FOREACH (diff IN row.differential | 
                    MERGE (d2:Disease {name: diff}) 
                    MERGE (d)-[:DIFFERENTIAL_DIAGNOSIS]->(d2))
            """, batch=batch_data)
        print(f"Đã nạp xong {len(batch_data)} bệnh và các mối quan hệ liên kết.")

    def process_drugs(self):
        folder = os.path.join(DATA_DIR, "drugs")
        if not os.path.exists(folder): return

        print("\n--- Đang nạp Thuốc (Drugs) ---")
        batch_data = []
        ...
        for filename in os.listdir(folder):
            ...
            meta = data.get('metadata', {})
            # Yêu cầu 4: Rich Drug Properties
            props = {
                ...
                "synonyms": meta.get('brand_names', []) # Brand names map to synonyms
            }
        ...
        with self.driver.session() as session:
            session.run("""
                ...
                // Đảm bảo chiều quan hệ Disease -> TREATED_BY -> Drug cho nhất quán
                WITH dr, row
            ...
            """, batch=batch_data)
        print(f"Đã nạp xong {len(batch_data)} loại thuốc.")

    def process_symptoms(self):
        """Bổ sung hàm xử lý Symptom riêng để nạp Rich Properties (Yêu cầu 3)"""
        folder = os.path.join(DATA_DIR, "symptoms")
        if not os.path.exists(folder): return

        print("\n--- Đang nạp Triệu chứng (Symptoms) ---")
        batch_data = []
        ...
        for filename in os.listdir(folder):
            ...
            batch_data.append({
                "name": meta.get('symptom_name', 'Unknown'),
                "props": props
            })
        ...
        with self.driver.session() as session:
            session.run("""
                ...
                SET s += row.props
            """, batch=batch_data)
        print(f"Đã nạp xong {len(batch_data)} triệu chứng đa dạng.")

def main():
    try:
        import neo4j
    except ImportError:
        print("Vui lòng chạy lệnh: pip install neo4j")
        return

    print("Bắt đầu kết nối Neo4j...")
    ...
    try:
        ...
        print("\n🎉 HOÀN TẤT! Siêu Đồ thị tri thức (GraphRAG) đã được xây dựng thành công.")
